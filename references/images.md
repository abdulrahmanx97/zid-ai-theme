...
- **Path B — المستخدم عنده صور على جهازه:** ask ONCE with an exact named list, never a vague "ارفع صور": deliver `ASSET_REQUIREMENTS.md` (prompt-packs.md format — filename · size · usage · priority) and say "ارفع لي هذه الملفات بهذه الأسماء". Files that come back drop straight into `assets/images/`.
- **Path C — ما عنده صور أبداً:** produce the generation prompt packs (`VISUAL_PROMPTS.md` / `VIDEO_PROMPTS.md` per prompt-packs.md). If the current platform can generate images/video natively (ChatGPT/Gemini — see platform-adapter.md), run the prompts yourself with the exact filenames; otherwise hand the pack to the user ("ولّد هذه في Gemini أو ChatGPT وارجع ارفعها لي بنفس الأسماء") and ship on-identity SVG placeholders meanwhile — the theme works NOW and upgrades when the files return.
