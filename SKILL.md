...
Never design blind. Build a store profile before any code: fetch the live storefront URL, and if the zid MCP connector is available, use its read tools (ListProducts, ListCategories, ListOrders for best sellers, ListProductReviews) to learn what the store sells, its real categories, and what deserves the hero. Then map the store to a sector identity (palette, font, layout language) and add one signature differentiator. Full process: `references/store-analysis.md` + `references/sector-identities.md`.
...
**Images: hybrid strategy.** Every banner/photo is a schema setting of type `image` (merchant replaces from the editor) + ship a default placeholder in `assets/images/` so the theme looks complete on first activation. Only identity constants (patterns, icons) are hardcoded in assets.
...
The skill's delivery contract: every theme task ends with an upload-ready ZIP handed to the user — never leave the work as loose files.
...
- `references/store-analysis.md` — profile the store via zid MCP + storefront fetch before designing. Read at Step 0, ALWAYS.
